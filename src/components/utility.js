...
export const negate = color => {
	const rgb = chroma(color).rgb();
	const neg = rgb.map((val, i) => 255 - val);
	return chroma(neg).hex();
};

export const getColor = (base, { multiplier, lightness, saturation }) => {
	...
	const isDull = chroma(neg).hsl()[1] < 0.5;
...
export const rotate = base => deg => {
	const [h, s, l] = chroma(base).hsl();
	return chroma(h + deg, s, l, 'hsl').hex();
};

export const getHues = base => length => {
	const hues = [];
...
export const hello = (base, options = {}) => {
	const {
	...
	console.log('hello returningggg', result);
	return result;
};
